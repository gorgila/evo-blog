...
const mutations = {
    ...
    ['updateAdminItem'](state, payload) {
        ...
        const index = state.lists.data.findIndex(item => item.id.toString() === payload.id.toString())
        if (index > -1) state.lists.data.splice(index, 1, payload)
    },
    ['deleteAdmin'](state, id) {
        const obj = state.lists.data.find(item => item.id === id)
